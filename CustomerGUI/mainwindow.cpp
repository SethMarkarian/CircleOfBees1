...
void MainWindow::on_s1_pb_CreateUser_clicked()
{
    ...
    if(email == "" || username == "" || first_name == "" || last_name == ""){
        ui->s1_l_status->setText("Please complete ALL fields");
    }else if(password.compare(conf_password) != 0)
    {
        ui->s1_l_status->setText("Passwords do not match!");
    }else{// this is where we'd add the username password combo to the database

        //MAKE SURE USERNAME DOESN'T ALREADY EXIST
        if(users->add_row(email, first_name, last_name, username, password) != 0){
            ui->s1_l_status->setText("Username is already taken");
        }else{
...
void MainWindow::on_s6_pb_runAlerts_clicked()
{
    /*Alert al;
